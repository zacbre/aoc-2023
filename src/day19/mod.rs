...
use crate::day19::workflows::{Operand, Var, Workflows};
...
fn run_workflow(workflows: &Workflows, workflow: &Workflow, x: usize, m: usize, a: usize, s: usize) -> (bool, Option<String>) {
    ...
    for (_index, step) in workflow.steps.iter().enumerate() {
        ...
        //println!("Workflow: {}, Step: {}, Part: {:?}, Part Value: {:?}, Operand: {:?}, To Check: {:?}", workflow.name, _index+1, step.var_to_check, part_value, step.operand, step.value_to_check);
...
#[cfg(test)]
mod tests {
    use crate::day19::{find_range, Range};
