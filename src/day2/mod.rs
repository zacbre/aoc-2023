...
struct Game {
    ...
    pub selections: Vec<CubeSelection>,
    pub possible: bool
}
...
fn parse_input(input: &str) -> Vec<Game> {
    ...
    for line in input.split('\n') {
        ...
        let game_number = parts.next().unwrap().split(' ').nth(1).unwrap().parse::<usize>().unwrap();
        game.game_number = game_number;

        // get the game selections.
        let selections = parts.next().unwrap().split(';');
        for selection in selections {
            let mut cube_selection = CubeSelection {
                red: 0,
                green: 0,
                blue: 0,
            };
            ...
            for part in parts {
                ...
                match color {
                    "red" => cube_selection.red = count,
                    "green" => cube_selection.green = count,
                    "blue" => cube_selection.blue = count,
                    _ => panic!("Unknown color: {}", color),
                }
            }
            game.selections.push(cube_selection);
        }
        games.push(game);
    }
    println!("Games: {:?}", games);
...
fn part_one(games: &mut Vec<Game>) {
    let mut possible_games: Vec<usize> = Vec::new();
    // 12 red, 13 green, 14 blue, if any games go above this, they are not possible.
    for game in games {
        ...
        if game.possible {
            possible_games.push(game.game_number);
        }
    }

    println!("Possible Games: {:?}", possible_games);

    // add up all the possible games into one number
    let mut total = 0;
...
fn part_two(games: &mut Vec<Game>) {
    // find the least amount of cubes allowed per each game
    let mut selection_totals: Vec<CubeSelection> = Vec::new();
    for game in games {
        let mut selection_total = CubeSelection {
            red: 0,
            green: 0,
            blue: 0,
        };
        for selection in &game.selections {
            if selection.red > selection_total.red {
                selection_total.red = selection.red;
            }
            if selection.green > selection_total.green {
                selection_total.green = selection.green;
            }
            if selection.blue > selection_total.blue {
                selection_total.blue = selection.blue;
            }
        }
        selection_totals.push(selection_total);
    }
    let mut total = 0;
    for selection in selection_totals {
        total += selection.red * selection.green * selection.blue;
    }
...
#[cfg(test)]
mod tests {
    use crate::day2::{parse_input, part_one, part_two};

    #[test]
    fn can_run() {
        let input = r"Game 1: 1 green, 6 red, 4 blue; 2 blue, 6 green, 7 red; 3 red, 4 blue, 6 green; 3 green; 3 blue, 2 green, 1 red
