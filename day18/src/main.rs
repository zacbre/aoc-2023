...
#[cfg(test)]
mod tests {
    use crate::{find_fill_start_point, input, parse, shoelace};

    #[test]
    fn can_parse_input() {
        let parsed = parse(input::EXAMPLE_INPUT, false);
        assert_eq!(parsed.len(), 14);
        assert_eq!(parsed[0].direction, crate::Direction::Right);
    ...
    fn can_get_grid_for_example_input() {
        let parsed = parse(input::EXAMPLE_INPUT, false);
        let ((min_x, max_x), (min_y, max_y)) = crate::find_dimensions(&parsed);
        let map = crate::get_grid((min_x, max_x), (min_y, max_y));
    ...
    fn can_draw_outline_for_example_input() {
        let parsed = parse(input::EXAMPLE_INPUT, false);
        let ((min_x, max_x), (min_y, max_y)) = crate::find_dimensions(&parsed);
        let mut map = crate::get_grid((min_x, max_x), (min_y, max_y));
        crate::solve_map(&mut map, &parsed, ((min_x, max_x), (min_y, max_y)));
    ...
    fn can_fill_outline_for_example_input() {
        ...
        crate::flood_fill(&mut map, find_fill_start_point(&cloned).unwrap());
        crate::draw_map(&map);
        let map = map.iter().map(|line| line.iter().collect::<String>()).collect::<Vec<String>>().join("\n");
    ...
    fn can_fill_outline_shoelace_for_input() {
        ...
        let ((min_x, max_x), (min_y, max_y)) = crate::find_dimensions_vertices(&parsed, &mut vertices);
